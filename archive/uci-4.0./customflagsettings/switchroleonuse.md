...
* `role_id` — `uint?` — Role identifier. If `role_type` is `Normal`, this should be a [`RoleTypeId`](../../../api/enums/external/roletypeid.md) enum value. If `role_type` is `UCR`/`ECR`, this should be the custom role ID. Default: `1`.
* `spawn_flags` — [`RoleSpawnFlags`](../../../api/enums/external/rolespawnflags.md)`?` — For `Normal` role types, controls how the new role spawns: `None`, `AssignInventory`, `UseSpawnpoint`, or `All`. Default: `None`.
