...
| `RoleId`                                    | A [`RoleTypeId`](../enums/external/roletypeid.md) enum value. Specifies the role (e.g., `Scientist`, `ClassD`, `Guard`, `FacilityGuard`) that the player will appear as when disguised. The available values correspond to the `PlayerRoles.RoleTypeId` enumeration in Unity/SCP:SL.                                                                    |
