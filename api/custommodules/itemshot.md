...
| `ItemType`               | The standard Unity/SCP:SL [`ItemType`](../enums/external/itemtype.md) to spawn. This property is only used if `IsCustomItem` is `false` and is used for both regular items and grenades (if `IsGrenade` is true). Example: `GrenadeHE`, `KeycardO5`. |
