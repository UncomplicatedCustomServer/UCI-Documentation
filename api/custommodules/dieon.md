...
| [`Trigger`](../enums/internal/triggeron.md) | Specifies when the module's effect should trigger. Possible values are `OnUse` (when the item is used) and `OnDropped` (when the item is dropped). Note that only one trigger can be configured per module instance. |
